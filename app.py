...
def register():
    if request.method=='POST':
        ...
        uuu = User.query.filter_by(username=username).first()
        if uuu and email == uuu.email:
            flash('Already registered','warning')
        else:    
            db.session.add(user)
            db.session.commit()
            flash('Congratualtion! You have succesfully registered','success')
...
def blogpost():
    if request.method=='POST':
        ...
        db.session.add(blog)
        db.session.commit()
        flash('Congratualtion! You have succesfully posted','success')
        return redirect('/')
